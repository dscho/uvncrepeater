...
BOOL CALLBACK DialogProc_small(HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
//	if ((Message!=WM_TIMER || Message!=WM_CLOSE) && scanner) return false;
  switch(Message)
  {
...
DWORD WINAPI DisplayIdWindow(LPVOID id)
{
	char buffer[200];
	sprintf(buffer,"Your ID for this connection is: %d",atoi((char *)id));
	MessageBox(NULL,buffer,"ID",MB_OK);
	return NULL;
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow)
{
	...
	if(smallgui)
	{
			...
			DialogBoxParam(hInstance, MAKEINTRESOURCE(IDC_DIALOG_SMALL), NULL, (DLGPROC)DialogProc_small,0);
	}
	else if(nogui)
	{
		if ((fid = fopen(menustrings, "r"))!=NULL)
	...
	for (i = 0; i < strlen(szCmdLine); i++)
	{
		...
		if (strncmp(&szCmdLine[i], winvncId, strlen(winvncId)) == 0) {
			...
			else if(fetchcode)
			{
				...
				if(strcspn(TextFETCHCODE,":")>250) exit(-1);
				if(strcspn(TextFETCHCODE+strcspn(TextFETCHCODE,":")+1,"\\/")>5) exit(-1);
				if(strlen(TextFETCHCODE)>1024) exit(-1);

				memcpy(hostname,TextFETCHCODE,(size_t)strcspn(TextFETCHCODE,":"));
				hostname[strcspn(TextFETCHCODE,":")]=(char)0;
				memcpy(temp_port,TextFETCHCODE+strcspn(TextFETCHCODE,":")+1,strcspn(TextFETCHCODE+strcspn(TextFETCHCODE,":")+1,"\\/"));
				temp_port[strcspn(TextFETCHCODE+strcspn(TextFETCHCODE,":")+1,"\\/")]=(char)0;
				
				memcpy(path,TextFETCHCODE+strlen(hostname)+strlen(temp_port)+1,strlen(TextFETCHCODE)-strlen(hostname)-strlen(temp_port));
				path[strlen(TextFETCHCODE)-strlen(hostname)-strlen(temp_port)]=(char)0;
				
				WSADATA wsaData;
				if(WSAStartup(MAKEWORD(1,1),&wsaData))
				{
					MessageBox(NULL,"Winsock init failed",NULL,MB_OK);
					exit (-1);
				}

				if(wsaData.wVersion != MAKEWORD(1,1)) 
				{
					MessageBox(NULL,"Winsock version not supported\nIs this an extremely old operating system?",NULL,MB_OK);
					WSACleanup();
					exit(-1);
				}

				IN_ADDR iaHost;
				LPHOSTENT lpHostEntry=(hostent *)malloc(sizeof(LPHOSTENT));
				...
				if(lpHostEntry == NULL)
				{
					MessageBox(NULL,"Can't resolve hostname",NULL,MB_OK);
					WSACleanup();
					exit(-1);
				}
				...
				{
					MessageBox(NULL,"Cant create socket",NULL,MB_OK);
					WSACleanup();
					exit(-1);
				}
				...
				saServer.sin_port = htons(atoi(temp_port));
				saServer.sin_family = AF_INET;
				saServer.sin_addr = *((LPIN_ADDR)*lpHostEntry->h_addr_list);

				if (connect(Socket, (LPSOCKADDR)&saServer, sizeof(SOCKADDR_IN)) == SOCKET_ERROR)
				{
					MessageBox(NULL,"Can't connect to fetch the connection ID",NULL,MB_OK);
					closesocket(Socket);
					WSACleanup();
					exit(-1);
				}
				...
				int nRet = send(Socket, szBuffer, strlen(szBuffer), 0);
				if (nRet == SOCKET_ERROR)
				{
					MessageBox(NULL,"Can't send data.",NULL,MB_OK);
					closesocket(Socket);
					WSACleanup();
					exit(-1);
				}
				...
				if (nRet == SOCKET_ERROR)
				{
					MessageBox(NULL,"Receive id failed",NULL,MB_OK);
					closesocket(Socket);
					WSACleanup();
					exit(-1);
				}

				if(strlen(szBuffer)<6) sprintf(g_idcode,"%d",atoi(szBuffer));
				else
				{
					MessageBox(NULL,"Server responded with unreadable ID",NULL,MB_OK);
					closesocket(Socket);
					WSACleanup();
					exit(-1);
				}
		...
		{
			DWORD th;
			CreateThread(NULL,0,DisplayIdWindow,(LPVOID)&g_idcode,0,&th);
		}
