...
class ListenerThread extends Thread {
    ...
    void processEvents(int numEvents) {
        ...
        for(int ii = 0; ii < numEvents; ii++) {
            ...
            while(slicedEvent.hasMoreTokens()) {
                ...
                StringTokenizer slicedField = new StringTokenizer(slices[numSlices][0], ":");
            ...
            try {
                ...
                switch(eventMeaning) {
                    case 0: //Viewer connect
                        Message = "Connection: code:" + slices[5][1] 
                            + ", Host:" + getHost(slices[7][1]);
                        serverTableInd = -1;
                    ...
                    case 4: //Viewer-Server session start
                        //Server message 1st
                        Message = "Active: code:" + slices[6][1] 
                            + ", Host:" + getHost(slices[8][1]);
                        viewerTableInd = -1;
                        serverTableInd = new Integer(slices[4][1]);
                        postMsgToGui(serverTableInd, viewerTableInd, Message);
    ...
    {
        try {
            ServerSocket serverSocket = new ServerSocket(listenPort, BACKLOG, InetAddress.getByName(listenIp));
...
public class EventListener extends JPanel {
    ...
    public EventListener(String listenIp, int listenPort) {
        ...
        ListenerThread thread = new ListenerThread(table, MAX_ROWS-1, listenIp, listenPort);
        thread.start();    
    }

    class MyTableModel extends AbstractTableModel {
        
        private String[] columnNames = {"TableIndex",
                                        "Servers",
                                        "Viewers"};
        private String[][] data;
    ...
    public static void main(String[] args) {
        ...
        try {
            if (args.length != 2) {
                System.out.println("Usage: java EventListener listenIp listenPort");
                System.out.println("listenIp: What interface we listen (default 0.0.0.0 == All interfaces)");
                System.out.println("listenPort: Port number where we listen (default 2002)");
