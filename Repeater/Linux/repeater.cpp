        ...
        if (fRemove) {
            ...
            if (useEventInterface) {
                ...
                if (connectionFrom == CONNECTION_FROM_VIEWER) {
                	event.eventNum = VIEWER_DISCONNECT;
                	uuid_copy(connEv.uid,viewers[index]->uid);
                } else {
                	event.eventNum = SERVER_DISCONNECT;
                	uuid_copy(connEv.uid,servers[index]->uid);
                }
...
void readPeerHandShake(int socket, int index)
{
    ...
    if (len < 0) {
       ...
       handShakes[index] -> handShakeLength = 0;
        ...
        debug(LEVEL_3, "readPeerHandShake(): len = %d,%s\n", len,handShakes[index] -> handShake);
                ...
                if (server == -1) {
                    ...
                    if ((fServerOk) && (fViewerOk)) {
                        ...
                        if (useEventInterface) {
                            ...
                            uuid_copy(connEv.uid,viewers[viewerInd]->uid);
                            memcpy(event.extraInfo, &connEv, sizeof(connectionEvent));
                            if (false == sendRepeaterEvent(event)) {
                            ...
                            uuid_copy(connEv.uid,servers[serverInd]->uid);
                            memcpy(event.extraInfo, &connEv, sizeof(connectionEvent));
                            if (false == sendRepeaterEvent(event)) {
                                debug(LEVEL_1, "acceptConnection(): Warning, event fifo is full\n"); 
                            }
                            ...
                            uuid_copy(sessEv.viewer_uid,viewers[viewerInd]->uid);
                            uuid_copy(sessEv.server_uid,servers[serverInd]->uid);
...
static void listInitializationValues(void)
{
    ...
    debug(LEVEL_2, "listInitializationValues(): mysqlHost : %s\n", mysqlHost);
    debug(LEVEL_2, "listInitializationValues(): mysqlPort : %d\n", mysqlPort);
    #endif
