...
char eventHandlerType[MY_TMP_BUF_LEN] = "eventlistener";

#ifdef EVENTS_USE_LISTENER
//EventListener hostname/ip address and port
char eventListenerHost[MY_TMP_BUF_LEN] = "localhost";
int eventListenerPort = 2002;
#endif

#ifdef EVENTS_USE_MYSQL
char mysqlHost[MY_TMP_BUF_LEN] = "localhost";
char mysqlUser[MY_TMP_BUF_LEN] = "user";
char mysqlPass[MY_TMP_BUF_LEN] = "password";
char mysqlDb[MY_TMP_BUF_LEN] = "events";
int mysqlPort = 3306;
#endif

#ifdef EVENTS_USE_SQLITE
char sqliteDbPath[] = "events.sqlite";
#endif
...
bool readIniFile(char *iniFilePathAndName)
{
    ...
    if (dict != NULL) {        
		...
		strlcpy(eventHandlerType, iniparser_getstring(dict, "eventinterface:eventhandlertype", "eventlistener"), MY_TMP_BUF_LEN);

		#ifdef EVENTS_USE_LISTENER
		strlcpy(eventListenerHost, iniparser_getstring(dict, "eventinterface:eventlistener_host", "localhost"), MY_TMP_BUF_LEN);
		eventListenerPort = iniparser_getint(dict, "eventinterface:eventlistener_port", 2002);
		#endif

		#ifdef EVENTS_USE_MYSQL
		strlcpy(mysqlHost, iniparser_getstring(dict, "eventinterface:mysql_host", "localhost"), MY_TMP_BUF_LEN);
		strlcpy(mysqlUser, iniparser_getstring(dict, "eventinterface:mysql_user", "user"), MY_TMP_BUF_LEN);
		strlcpy(mysqlPass, iniparser_getstring(dict, "eventinterface:mysql_pass", "password"), MY_TMP_BUF_LEN);
		strlcpy(mysqlDb, iniparser_getstring(dict, "eventinterface:mysql_db", "password"), MY_TMP_BUF_LEN);
		mysqlPort = iniparser_getint(dict, "eventinterface:mysql_port", 3306);
		#endif
		
		#ifdef EVENTS_USE_SQLITE
		strlcpy(sqliteDbPath, iniparser_getstring(dict, "eventinterface:sqlite_dbpath", "events.sqlite"), MY_TMP_BUF_LEN);
		#endif
		#endif
